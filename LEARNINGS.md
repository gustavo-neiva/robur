...
Rebirth — the .ratchet* → .robur* rename (2026-09-04):
- A rename is only scary when the names are string literals. They were, in 21 files; `Robur::Paths` is now the single source of truth and every on-disk name (`.robur/`, `.robur.conf`, `~/.robur/`, `ROBUR_HOME`/`ROBUR_METRICS`/`ROBUR_LOOP`) comes from a constant there. Never hardcode one again — the next rename should be a constant edit.
- The compatibility shape that made the rename free: READ the new name first and fall back to the legacy one, WRITE the new name and leave `.ratchet` as a relative symlink to `.robur`. External readers (`atlas/bin/money-loop.sh`, `status.sh`, harbor's `/blocked`) follow the symlink and were not touched at all. Cost: one symlink.
- A symlink only rescues a DIRECTORY. `.ratchet.conf` is a plain file with no shim, so `money-loop.sh:91`'s `is_runnable()` — which gates on `.ratchet.conf` existing — is the one estate reference the rename does not cover for a repo initialized fresh onto `.robur.conf`. Flagged in REWIRING.md §5; found by re-reading the caller, not by grepping our own repo.
- `Paths.link_legacy!` must refuse to replace a REAL `.ratchet` directory, because `money-loop.sh:81` does `mkdir -p "$repo/.ratchet"` before writing backoff. A repo where the supervisor writes first stays on the legacy layout and keeps its data — correct, but it means "the symlink is always there" is not an invariant you can rely on.
- `~/.robur/` gets NO symlink shim: `Paths.home` just keeps resolving to a pre-existing `~/.ratchet/`. On this machine `~/.robur` does not exist, so the global conf, logs and `metrics.tsv` never moved and `status.sh` never noticed. Check what the resolver actually returns before documenting a path as changed.
- Parity against the bash original is RETIRED. The differential gate is being removed; the unit suite is the only gate. Documentation that justified a choice as "a deliberate divergence from bash" had to be rewritten to justify itself on its own terms — if an entry could not survive that rewrite it was never a design decision, only a diff.
- Provenance comments (`# port of run-turn.sh:81`) are a liability once the original stops being the oracle: they point at a repo readers can no longer consult. Delete the citation, keep the reason, rewrite the sentence to stand alone.
- Docs age against the estate, not against the repo. Every file:line in REWIRING.md was re-verified by reading `atlas/` and `harbor/` again rather than trusting the previous pass — that is how the `is_runnable()` gap surfaced, and it is not something a grep of our own repo could ever have shown.
